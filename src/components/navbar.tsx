    ...
    <header className="flex h-20 w-full justify-between shrink-0 items-center px-4 md:px-6">
      <Sheet>
        ...
        <SheetContent side="left">
          <Link href="/" className="mr-6 hidden lg:flex gap-2" prefetch={false}>
            <MountainIcon className="h-6 w-6" />
            <span className="text-lg font-semibold">ExplorArte</span>
          </Link>
          <div className="grid gap-2 py-6">
            {session?.user && (
              <Link href="/search" className="flex w-full items-center py-2 text-lg font-semibold" prefetch={false}>
                Search
              </Link>
            )}
            {session?.user && (
            ...
            {session?.user && (
              <Button variant="destructive" className="h-9" onClick={() => signOut()}>
                Sign out
              </Button>
            )}
